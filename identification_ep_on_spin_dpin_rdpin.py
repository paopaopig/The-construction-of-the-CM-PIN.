...
Dis=np.load('model_detection/%s/%s/%s/%s.npy'%(species,dataset,pin,pin))
nodes_original=np.load('model_detection/%s/%s/nodes_name.npy'%(species,dataset))
essential_nodes_name=np.load(r"model_detection/%s/%s/ess_gene_name.npy"%(species,dataset))#关键蛋白节点的名字
gene_value = np.load(r'model_detection/%s/%s/gene_value_profile.npy'%(species,dataset))


lc = pec(Dis, nodes_original, gene_value)
